...
resumeRouter.post('/', upload.single('resume'), async (req, res) => {
    ...
    try {
        ...
        db.prepare(`INSERT INTO resume (id, filename, text, terms, uploaded_at)
       VALUES (1, ?, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET
         filename = excluded.filename,
         text = excluded.text,
         terms = excluded.terms,
         uploaded_at = excluded.uploaded_at`).run(req.file.originalname, text, JSON.stringify(terms), uploadedAt);
