...
export async function refreshJobs() {
    ...
    try {
        ...
        for (const r of results) {
            if (r.status === 'fulfilled')
                fetched = fetched.concat(r.value);
            else
                console.error('[refresh] fetcher failed:', r.reason);
        }
    }
    catch (err) {
        errorMsg = err instanceof Error ? err.message : String(err);
    }
    const resume = db.prepare('SELECT text FROM resume WHERE id = 1').get();
    let scores = new Map();
    if (resume?.text) {
  ...
  `);
    const countBefore = db.prepare('SELECT COUNT(*) as c FROM jobs').get().c;
    ...
    const countAfter = db.prepare('SELECT COUNT(*) as c FROM jobs').get().c;
    const added = countAfter - countBefore;
    db.prepare('INSERT INTO refresh_log (ran_at, fetched_count, new_count, error) VALUES (?, ?, ?, ?)').run(ranAt, fetched.length, added, errorMsg);
...
export function rescoreAll() {
    ...
    const tx = db.transaction(() => {
        for (const j of jobs) {
            ...
            if (m)
                stmt.run(m.score, JSON.stringify(m.matchedTerms), j.id);
        }
    });
    tx();
    return jobs.length;
